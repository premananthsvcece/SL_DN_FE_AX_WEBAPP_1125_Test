  ...
  useEffect(() => {
    if (data && data.state && stateOptions && stateOptions.length > 0) {
      ...
      if (state) {
        ...
        const initialCity = citiesData.find(
...
useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.street) errors.push("Street required");
...
if (key === "type")
  return (
    ...
    />
                ...
                if (key === "referred_by") {
                  if (formData.reference !== "Customer reference")
                    return null;
